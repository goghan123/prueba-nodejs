...
let botonIngresadorIntegrantes = document.querySelector('#ingresador-integrantes');
let botonCalculador = document.createElement("button");
let listaEdades = [];
let inputMayorEdad = document.createElement("input");
let inputMenorEdad = document.createElement("input");
let inputPromedioEdad = document.createElement("input");
let hola3 = 0;

botonIngresadorIntegrantes.onclick = function ingresarIntegrantes(){
    
    
    var numeroIntegrantes = Number(document.querySelector('#numero-integrantes').value);
    
    //crear etiqueta de mayor edad
    let etiquetaMayorEdad = document.createElement("label");
    let textoEtiquetaMayorEdad = document.createTextNode("Edad mayor: ");
    etiquetaMayorEdad.appendChild(textoEtiquetaMayorEdad);
    document.body.appendChild(etiquetaMayorEdad);
    
    //crear input de mayor edad
    inputMayorEdad.disabled = true;
    document.body.appendChild(inputMayorEdad);
    
    //crear label de menor edad
    let etiquetaMenorEdad = document.createElement("label");
    let textoEtiquetaMenorEdad = document.createTextNode(" Edad menor: ");
    etiquetaMenorEdad.appendChild(textoEtiquetaMenorEdad);
    document.body.appendChild(etiquetaMenorEdad);
    
    //crear input de menor edad
    inputMenorEdad.disabled = true;
    document.body.appendChild(inputMenorEdad);
    
    //crear label promedio de edad
    let etiquetaPromedioEdad = document.createElement("label");
    let textoEtiquetaPromedioEdad = document.createTextNode(" Edad promedio: ");
    etiquetaPromedioEdad.appendChild(textoEtiquetaPromedioEdad);
    document.body.appendChild(etiquetaPromedioEdad);
    
    //crear input promedio de edad
    inputPromedioEdad.disabled = true;
    document.body.appendChild(inputPromedioEdad);
    
    for (let i = 1; i <= numeroIntegrantes; i++){
        crearEtiquetaFamiliar(i);
        crearInputFamiliar(i);
    }
    crearBotonCalculador()
    return false;
}

function crearEtiquetaFamiliar(i){
    let etiquetaBaseFamiliar = document.createElement("label");
    let nombreEtiquetaBaseFamiliar = document.createTextNode(" Familiar " + i + ": ");
    etiquetaBaseFamiliar.appendChild(nombreEtiquetaBaseFamiliar);
    document.body.appendChild(etiquetaBaseFamiliar);
}

function crearInputFamiliar(i){
    let inputBaseFamiliar = document.createElement("input");
    inputBaseFamiliar.setAttribute('id', 'prueba-de-atributo' + i);
    inputBaseFamiliar.setAttribute('type', 'number');
    document.body.appendChild(inputBaseFamiliar);
}

function crearBotonCalculador(){
    //let textoBotonCalculador = document.createTextNode("Calcular");
    botonCalculador.innerHTML = 'Calcular';
    document.body.appendChild(botonCalculador);
}

botonCalculador.onclick = function efectoBotonCalculador(){
    let mismaCantidadIntegrantes = Number(document.querySelector('#numero-integrantes').value);
    for ( e = 1 ; e <= mismaCantidadIntegrantes; e++){
    let edadIngresada = Number(document.querySelector('#prueba-de-atributo'+e).value);
    listaEdades.push(Number(edadIngresada));
}
    calcularMayorEdad();
    calcularMenorEdad();
...
function calcularMayorEdad(){
    inputMayorEdad.disabled = false;
    inputMayorEdad.value = Math.max(...listaEdades);
}

function calcularMenorEdad(){
    inputMenorEdad.disabled = false;
    inputMenorEdad.value = Math.min(...listaEdades);
}

function calcularEdadPromedio(){
    inputPromedioEdad.disabled = false;
    for(hola2=0; hola2 < listaEdades.length; hola2++){
    hola3 = hola3 + Number(listaEdades[hola2]);
    }
    //hola3= resultado de la sumatoria de edades
    let edadPromedio = hola3 / listaEdades.length;
    inputPromedioEdad.value = edadPromedio;
}   
